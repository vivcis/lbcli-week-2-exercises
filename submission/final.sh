...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r '.vout[] | select(.scriptPubKey.address=="'$PAYMENT_ADDRESS'") | .value' | awk '{printf "%.8f", $1}')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r '.vout[] | select(.scriptPubKey.address=="'$CHANGE_ADDRESS'") | .value' | awk '{printf "%.8f", $1}')
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq -r '.vout[] | select(.scriptPubKey.address=="'$CHANGE_ADDRESS'") | .n')
...
# Sequence number = 10 for a 10-block CSV relative timelock
# WRITE YOUR SOLUTION BELOW:
...
# Version 2 is required for CSV (BIP 68) enforcement
TIMELOCK_TX=$(bitcoin-cli -regtest createrawtransaction "$TIMELOCK_INPUTS" "$TIMELOCK_OUTPUTS" 0 2)
